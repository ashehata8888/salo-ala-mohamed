...
function App() {
  ...
  const syncVoiceSettings = async (updates: any) => {
    ...
    if (updates.voiceFrequency !== undefined) await rescheduleSalahNotifications();
  };
          ...
          <div className="tab-content">
            ...
            {isHourlyVoiceEnabled && (
              <div className="voice-advanced-controls">
                ...
                {/* iOS plays the clip as a notification sound, which follows
                    the ringer volume; an app-level volume cannot apply there. */}
                {isIos ? (
                <div className="control-item col-layout">
                  <label className="vol-label">
                    {isRtl
                      ? "يُشغَّل الصوت بمستوى صوت الرنين في هاتفك"
                      : "Plays at your iPhone's ringer volume"}
                  </label>
                  <button className="add-schedule-btn" onClick={() => handleTestVolume(1)}>
                    {isRtl ? "استمع للتذكير" : "Preview sound"}
                  </button>
                </div>
                ) : (
                <div className="control-item col-layout">
